"""
macOS Keychain storage via the keyring library.

Stores/retrieves X25519 keys and the derived shared secret.
All values are base64-encoded before storage — never plaintext key material in logs.

See docs/ARCHITECTURE.md §3.1 for key management spec.
"""
...
def has_completed_setup() -> bool:
    """True if all four keys are present in the keychain."""
    return all([
        load_private_key(),
        load_public_key(),
